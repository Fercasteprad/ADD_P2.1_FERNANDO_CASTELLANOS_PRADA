...
import dam2.add.p21.service.UsuariosService;
import dam2.add.p21.util.ConfigService;
...
public class Controller extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String persistencia = request.getParameter("persistencia");
		...
		if (persistencia != null) {
			...
			ConfigService.establecerPersistencia(persistencia);
			Properties prueba = ConfigService.modeloPersistencia();
			String parametro = prueba.getProperty("persistencia");
			log.info("Persistencia cambiada a "+parametro);
			...
			request.getRequestDispatcher("/views/persistencia.jsp").forward(request, response);
		}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (valor_email != null) {
			...
			if (UsuariosService.existeCorreo(valor_email)) { // Si el correo existe manda mensaje de error
				...
				Usuario nuevoUsuario = new Usuario(valor_nombre, valor_apellido, valor_email, telefono, valor_pass,
						valor_pass2);
		...
		if (id != null) {
			...
			// Si viene contraseña es que quieren editar contraseña
				...
				if (!(usuarioEdicion.getEmail().equals(emailEdit)) && UsuariosService.existeCorreo(emailEdit)) {
					...
					request.getSession().setAttribute("id", null);
					request.getSession().setAttribute("confirmar", "El usuario ha sido actualizado");
					log.debug("Datos actualizados de "+usuarioEdicion.getNombre()+" por el administrador "+usuarioSesion.getNombre());
					response.sendRedirect(request.getContextPath() + "/views/home.jsp");
